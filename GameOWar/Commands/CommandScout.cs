...
internal class CommandScout : Command
{
    ...
    public override void StartMethod()
    {
        ...
        base.StartMethod();
        var player = BotManager.Instance.Game.WorldMap.FindPlayer(_playerBase.Owner);

        if (_playerBase.IsPerformingAction && !_isPerformingBecauseOfMe)
        {
            _ = Task.Run(async () => await BotManager.Instance.SendMessage($"{player.UserName}'s {_playerBase.BaseName} is already scouting/attacking!"));
            return;
        }

        if (string.IsNullOrEmpty(_args))
            _ = Task.Run(async () => await BotManager.Instance.SendMessage($"{player.UserName}'s {_playerBase.BaseName} is scouting area in {ticksRemaining} days."));
        else
            _ = Task.Run(async () => await BotManager.Instance.SendMessage($"{player.UserName}'s {_playerBase.BaseName} is scouting {_args} in {ticksRemaining} days."));
    }
    ...
    public override void OnTick()
    {
        ...
        {
            BotManager.Instance.QueueMessage($"{player.UserName}'s {_playerBase.BaseName} is halfway done scouting {_args}... {ticksRemaining} days remaining.");
            _isHalfwayBuilt = true;
        }

        if (ticksRemaining < _startDuration / 5 && !_isAlmostBuilt)
        {
            BotManager.Instance.QueueMessage($"{player.UserName}'s {_playerBase.BaseName} is almost done scouting {_args}... {ticksRemaining} days remaining.");
            _isAlmostBuilt = true;
        }
    }
        ...
        if (!string.IsNullOrEmpty(_args))
        {
            ...
            foreach (var b in _map.WorldBases)
            {
                if(b.BaseName.ToLower().Contains(_args))
                {
                    ...
                    var player = BotManager.Instance.Game.WorldMap.FindPlayer(_playerBase.Owner);
                    player.Knowledge.AddBaseKnowledge(b);
                    foundResult = true;
                    break;
                }
            }
